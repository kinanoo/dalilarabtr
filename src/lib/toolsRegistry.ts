...
export interface Tool {
  ...
  href: string;
  icon: LucideIcon;
  /** Tailwind gradient (from-… to-…) for the icon chip. Deliberately the SAME
   *  brand ramp for every tool: thirteen tools once carried thirteen gradients,
   *  which turned the hub into a swatch board and taught the reader nothing —
   *  the icon and title identify the tool, colour does not. */
  color: string;
  group: ToolGroupId;
