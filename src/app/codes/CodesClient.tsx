...
export default function CodesClient({ initialCodes }: { initialCodes: AdminCode[] }) {
  const { codes: remoteCodes, loading } = useAdminCodes();
  // The server already passed the full code list, so render it INSTANTLY — no
  // client-side spinner wall (the old behaviour blocked the whole list behind a
  // SWR fetch, which on slow / low-end devices showed a long spinner). Remote
  // data merges in silently once SWR resolves.
