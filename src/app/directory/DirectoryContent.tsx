...
export default function DirectoryContent({ initialArticles = [] }: { initialArticles?: DirectoryArticle[] }) {
  ...
  const allArticles = useMemo((): DirectoryArticle[] => {
    ...
    const merged = [...articles, ...scenarios];

    // 🌱 SSR seed: until the client SWR data arrives, render the server-fetched
    // list so the primary content is present in the very first HTML (crawlable)
    // instead of an empty shell behind a spinner. Once remote data resolves it
    // takes over transparently.
    if (merged.length === 0 && initialArticles.length > 0) {
      return initialArticles;
    }

    return merged;
  }, [adminArticles, adminScenarios, initialArticles]);
  ...
  // Only block the whole page on the spinner when we have NOTHING to show yet.
  // When the server passed `initialArticles`, `allArticles` is already populated
  // so this branch is skipped and the full list renders in the initial HTML.
