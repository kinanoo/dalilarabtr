...
export default function ResidencePage() {
  // تصفية المقالات الخاصة بالإقامة فقط
  const articles = Object.entries(ARTICLES).filter(([key, data]) => data.category === 'الإقامة والأوراق');

  return (
    <main className="flex flex-col min-h-screen">
      <Navbar />
      <PageHero
        title="الإقامة والأوراق القانونية"
        description="دليلك الكامل لفهم أنواع الإقامات وتجنب المخالفات."
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {articles.length > 0 ? (
            ...
            <div className="col-span-3 text-center py-12 text-slate-500 dark:text-slate-300 bg-slate-100 dark:bg-slate-900 rounded-lg border border-dashed border-slate-300 dark:border-slate-700">
              لا توجد مقالات مضافة في هذا القسم حالياً. يرجى تحديث قاعدة البيانات.
            </div>
          )}
        </div>
      </div>
