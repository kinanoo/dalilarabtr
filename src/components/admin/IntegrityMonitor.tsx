    ...
    return (
        <div className="space-y-8">
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                <div>
                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-rose-100 dark:bg-rose-900/30 text-rose-700 dark:text-rose-300 rounded-full text-[10px] font-black tracking-wider uppercase mb-2">
                        <ShieldCheck size={12} />
                        تشخيص
                    </span>
                    ...
                    <p className="text-slate-500 mt-1.5">نظام مسح آلي للكشف عن أي خلل في جودة البيانات (Data Integrity)</p>
                </div>

                <button
                    onClick={runScan}
                    disabled={loading}
                    className={`group/btn flex items-center gap-2 px-6 py-3 rounded-xl font-black text-white transition-all active:scale-95 shadow-lg shadow-emerald-500/30 hover:shadow-xl hover:shadow-emerald-500/40 hover:-translate-y-0.5 ${loading ? 'bg-slate-400 cursor-not-allowed hover:translate-y-0 hover:shadow-lg' : 'bg-gradient-to-l from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700'}`}
                >
                    <RefreshCw size={20} className={loading ? 'animate-spin' : 'group-hover/btn:rotate-180 transition-transform duration-500'} />
            ...
            {report && (
                <motion.div
                    ...
                    className="space-y-6"
                >
                    {/* Score Card — magazine spread with accent stripe */}
                    <div className="bg-gradient-to-br from-white to-slate-50/60 dark:from-slate-900 dark:to-slate-800/40 rounded-3xl p-8 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col md:flex-row items-center justify-between gap-8 relative overflow-hidden">
                        {/* Accent stripe — right edge in RTL, color reacts to score */}
                        <span className={`absolute top-0 right-0 h-full w-1.5 opacity-80 pointer-events-none ${healthScore > 80 ? 'bg-gradient-to-b from-emerald-500 to-teal-500' : healthScore > 50 ? 'bg-gradient-to-b from-amber-400 to-amber-500' : 'bg-gradient-to-b from-red-500 to-rose-500'}`} />

                        <div className="flex-1 text-center md:text-right z-10">
                            <h3 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-2.5">مؤشر جودة النظام</h3>
                            <div className="flex items-baseline gap-2 justify-center md:justify-start">
                                <span className={`text-6xl font-black tabular-nums leading-none ${healthScore > 80 ? 'text-emerald-500' : healthScore > 50 ? 'text-amber-500' : 'text-red-500'}`} dir="ltr">
                                    {healthScore}%
                                </span>
                                <span className="text-slate-400 font-bold tabular-nums" dir="ltr">/ 100</span>
                            </div>
                        ...
                        <div className="flex gap-3 md:gap-4 z-10">
                            <div className="text-center px-5 py-3 rounded-2xl bg-white/80 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 shadow-sm">
                                <div className="text-3xl font-black text-slate-800 dark:text-white tabular-nums" dir="ltr">{totalIssues}</div>
                                <div className="text-[10px] text-slate-500 font-black uppercase mt-1 tracking-wider">مشكلة نشطة</div>
                            </div>
                            <div className="text-center px-5 py-3 rounded-2xl bg-white/80 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 shadow-sm">
                                <div className="text-3xl font-black text-slate-800 dark:text-white tabular-nums" dir="ltr">{ignoredWithIssues.length}</div>
                                <div className="text-[10px] text-slate-500 font-black uppercase mt-1 tracking-wider">فئة متجاهَلة</div>
                            </div>
                        </div>

                        <div className={`absolute -left-10 -bottom-10 w-64 h-64 rounded-full blur-3xl opacity-30 pointer-events-none ${healthScore > 80 ? 'bg-emerald-300' : healthScore > 50 ? 'bg-amber-300' : 'bg-red-300'}`} />
                    </div>
