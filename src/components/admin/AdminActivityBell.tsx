...
// The bell answers ONE question: "what needs me right now?" — so it lists only
// events generated by OTHER people that await the admin's response (a member
// signed up, a service/scenario was submitted, a comment/review landed). The
// admin's OWN publishing (articles, faqs, codes, tools, updates, zones) is not
// news to the admin who just published it — those events are logged for the
// record but never shown here, or the feed becomes a useless echo of your own
// clicks. Filtered at the query + realtime layers so the badge counts signal only.
const SIGNAL_EVENTS = ['new_member', 'new_service', 'new_comment', 'new_review', 'new_scenario'];
    ...
    useEffect(() => {
        ...
        const channel = supabase
            ...
            .on('postgres_changes', {
                ...
                // Ignore the admin's own publishing events — signal only.
                if (!SIGNAL_EVENTS.includes(newEvent.event_type)) return;
                        ...
                        <div className="flex-1 overflow-y-auto">
                            {loading ? (
                                ...
                                <div className="p-8 text-center">
                                    ...
                                    <p className="text-slate-500 dark:text-slate-400 font-medium">لا شيء يحتاج انتباهك الآن ✅</p>
                                    <p className="text-slate-400 dark:text-slate-500 text-xs mt-1">أعضاء وخدمات وتعليقات جديدة تظهر هنا</p>
                                </div>
                            ) : (
