...
import { Settings, Menu, Layers, Save, Loader2, RefreshCw, Trash2, Edit, Plus, Check, X } from 'lucide-react';
...
export default function ConfigManager() {
    const [activeTab, setActiveTab] = useState<'menus' | 'categories' | 'migration'>('menus');
                                ...
                                <div key={menu.id} className="p-3 flex flex-wrap items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800/50 gap-2">
                                    {editingMenu === menu.id ? (
                                        <div className="flex flex-1 gap-2 items-center">
                                            ...
                                            />
                                            <input
                                                className="border rounded px-2 py-1 flex-1 text-sm ltr"
                                                value={menuForm.href}
            ...
            {activeTab === 'categories' && (
                <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
                    ...
                    {loading ? (
                        ...
                        <div className="divide-y divide-slate-100 dark:divide-slate-800">
                            {categories.map(cat => (
                                <div key={cat.slug} className={`p-4 flex items-center justify-between ${!cat.active && 'opacity-50 grayscale'}`}>
                                    {editingCategory === cat.slug ? (
                                        <div className="flex flex-1 gap-2 items-center">
                                            ...
                                            <button onClick={() => setEditingCategory(null)} className="p-1 bg-red-100 text-red-600 rounded"><X size={16} /></button>
                                        </div>
                                    ) : (
                                        <>
                                            ...
                                            <div className="flex items-center gap-2">
                                                <button
                                                    ...
                                                    className={`text-xs font-bold px-2 py-1 rounded ${cat.active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}
                                                >
                                                    {cat.active ? 'نشط' : 'معطل'}
                                                </button>
                                                <button
