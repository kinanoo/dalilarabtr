-- Generated from data/service-directory/batches/2026-07-priority-02.json.
-- Publishes only source-checked candidates that are not already in the directory.

WITH target_batch AS (
  SELECT id
  FROM public.service_import_batches
  WHERE label = 'الدفعة الثانية - أنقرة وأنطاليا وأضنة وبورصة - 2026-07-30'
...
BEGIN
  ...
  WHERE b.label = 'الدفعة الثانية - أنقرة وأنطاليا وأضنة وبورصة - 2026-07-30'
    AND c.status = 'imported';

  IF imported_count <> 15 THEN
    RAISE EXCEPTION
      'Expected 15 imported service candidates, found %',
      imported_count;
  END IF;
END $$;
