...
BEGIN
  ...
  FOR f IN
    SELECT * FROM (VALUES
      ...
      ('excerpt',         $ex$خريطة واحدة لسيارتك في تركيا 2026: التأمين الإلزامي والشامل، المخالفات وخصم 25% خلال شهر، رسوم الطرق HGS ومهلة الـ15 يوماً، المعاينة الدورية، وضريبة MTV — بالأرقام الرسمية والقنوات المجانية للاستعلام.$ex$),
      ('seo_title',       $st$سيارتك في تركيا 2026: التأمين والمخالفات وHGS والمعاينة — دليل شامل بالأرقام$st$),
      ('seo_description', $sd$كل ما يخصّ سيارتك في تركيا 2026: غرامات المرور المحدَّثة، خصم 25% خلال شهر لا 15 يوماً، مهلة HGS قبل تضاعف الغرامة، رسوم المعاينة، أقساط MTV، ورسم النوتر النسبي الجديد. مع القنوات الرسمية المجانية للاستعلام.$sd$)
    ) AS t(col, val)
  LOOP
    -- التحويل إلى نوع العمود الفعلي لا إلى نوع مفترض: last_update في القاعدة
    -- من نوع date، وتمرير نصّ إليه بلا تحويل يرفع
    -- «column is of type date but expression is of type text». الحرفيّة غير
    -- المُعلَمة كانت تُقبل ضمنياً؛ المُعلَمة لا تُقبل — وهذا ما أفشل التشغيل.
  ...
  FOR f IN
    SELECT * FROM (VALUES
      ...
      ('seo_keywords', ARRAY['تأمين السيارات تركيا','مخالفات المرور 2026','HGS','معاينة السيارة','MTV','رخصة القيادة','لوحات MA']::text[])
    ) AS t(col, val)
  LOOP
